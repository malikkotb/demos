...
void main() {
    ...
    // vec2 wavedUv = vec2(
    //     vUv.x + sin(vUv.y * 50.0 + uTime * 2.0) * 0.1,
    //     vUv.y + sin(vUv.x * 50.0 + uTime * 3.0) * 0.1 // different speeds
    // );
    // float strength = 1.0 - step(0.01, abs(distance(wavedUv, vec2(0.5)) - 0.25));

    // Pattern 40 - angle of vUv
    ...
    // Pattern 45 - sinusoid waves effect on a circle
    ...
    // perlin noise can be used to recreate natura shapes like clouds, water, fire, terrain, elevation
    // it can also be used to animate the grass or snow moving in the wind

    // Classic Perlin noise - Stefan Gustavson implementation
    // we provide a vec2 and we get a float in return
    // float strength = cnoise(vUv * 10.0);


    // Pattern 47 - perlin noise with step function
    // float strength =step(0.0, cnoise(vUv * 10.0));

    // Pattern 48 - perlin noise with
    // float strength = 1.0 - abs(cnoise(vUv * 10.0));

    // Pattern 49 - prlin nnoise with sin function
    // float strength = sin(cnoise(vUv * 10.0) * 20.0);

    // Pattern 50 - sharper perlin noise with sin function, using step function to create a sharp edge
